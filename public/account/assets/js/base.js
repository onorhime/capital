//-----------------------------------------------------------------------
// Version:        2.2.1
// Template name:  Dmgblockchain - Wallet & Banking HTML Mobile Template
// Item URL :      https://themeforest.net/item/Dmgblockchain-wallet-banking-html-mobile-template/25738217
// Author:         Bragher
...
goBackButton.forEach(function (el) {
    el.addEventListener("click", function () {
        if (Dmgblockchain.Animation.goBack) {
            goBackAnimation();
        }
        else {
            window.history.go(-1);
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("DmgblockchainAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("DmgblockchainAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("DmgblockchainDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (Dmgblockchain.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (Dmgblockchain.Dark_Mode.local_mode.enable) {
    var nightStart = Dmgblockchain.Dark_Mode.local_mode.start_time;
    var nightEnd = Dmgblockchain.Dark_Mode.local_mode.end_time;
...
if (Dmgblockchain.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("DmgblockchainDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("DmgblockchainDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("DmgblockchainDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
if (document.querySelector(".offcanvas") === null) {
    ...
    var CookiesStatus = localStorage.getItem("DmgblockchainCookiesStatus")
    ...
    document.querySelectorAll(".accept-cookies").forEach(function (el) {
        el.addEventListener("click", function () {
            localStorage.setItem("DmgblockchainCookiesStatus", "1")
        })
    })
}
...
function testMode() {
    ...
    console.clear();
    console.log("%cDmgblockchain", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #0f8f62; padding: 10px 120px; margin-bottom: 16px;")
    ...
    function testModeInfo(value, msg) {
        console.log("%c|" + "%c " + msg + " : " + "%c" + value, "color: #444; font-size :1.2em; font-weight: bold;", "color: inherit", "color:#0f8f62; font-weight: bold;");
    }
    ...
    testModeMsg(Dmgblockchain.PWA.enable, "PWA")
    testModeMsg(Dmgblockchain.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(Dmgblockchain.Dark_Mode.local_mode.enable, "Local dark mode (between " + Dmgblockchain.Dark_Mode.local_mode.start_time + ":00 and " + Dmgblockchain.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(Dmgblockchain.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(Dmgblockchain.RTL.enable, "RTL")
    testModeMsg(Dmgblockchain.Test.enable, "Test mode")
    testModeMsg(Dmgblockchain.Test.alert, "Test mode alert")
    ...
    testModeMsg(Dmgblockchain.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = Dmgblockchain.Test.word;
    var value = "";
        ...
        if (value == word || value === word) {
            ...
            if (Dmgblockchain.Test.alert) {
                ...
                var alert =
                    ...
                    +
                    Dmgblockchain.Test.alertMessage
                    +
                    "</strong></div></div></div>"

...
$sql5 = "SELECT * FROM investments WHERE status = 'progress' ORDER BY id DESC";
$trade = mysqli_query($conn, $sql5);
...
$fbal = $user['0']['balance'];
$nbal = $fbal + $toadd + $amount;

$sql = "UPDATE users SET balance = $nbal WHERE uid = '$uid'";
     
     
     mysqli_query($conn, $sql);
     
  
     $sql = "UPDATE investments SET status = 'Completed' WHERE id = $id";
                ...
                <nav class="navbar navbar-expand-lg navbar-light">
                    ...
                    <div class="collapse navbar-collapse" id="navbarNav">
                        <ul class="navbar-nav flex-column">
                            ...
                            <li class="nav-item ">
                                <a class="nav-link" href="investment.php" aria-controls="submenu-4"><i class="fab fa-fw fa-wpforms"></i>Investments</a>
                                    ...
                                    <div class="card-body p-0">
                                        <div class="table-responsive">
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    <?php foreach ($trade as $user): ?>
                                                   <tr>
                                                        <?php if ($user['status'] == "progress") {
                                                           echo('<td><span class="badge-dot badge-success mr-1"></span> In progress </td>');
                                                        }else{
                                                            echo('<td><span class="badge-dot badge-brand mr-1"></span> Complete </td>');
                                                        }?>
                                                
                                                        <td> <a href="profile.php?uid=<?php echo $user['uid'];?>" target="_blank" rel="noopener noreferrer"><?php echo $user['name'];?></a></td>
                                                        <td><?php echo $user['amount'];?> </td>
                                                        <td><?php echo $user['plan'];?> </td>
                                                        <td><?php echo $user['date'];?></td>
                                                        <td><?php  if($user['status']=="progress"):?>
                                                            <form action="" method="post"><input type="text" name="id" value="<?php echo $user['id'];?>" hidden>
                                                            <input type="text" name="uid" value="<?php echo $user['uid'];?>" hidden>
                                                            <input type="text" name="plan" value="<?php echo $user['plan'];?>" hidden>
                                                            <input type="text" name="rate" value="<?php echo $user['rate'];?>" hidden>
                                                            <input type="text" name="duration" value="<?php echo $user['duration'];?>" hidden>
                                                            <input type="text" name="amount" value="<?php echo $user['amount'];?>" hidden>
                                                            <input type="submit" class="btn btn-primary btn-lg btn-block" name="complete" value="complete trade">
                                                            <input type="submit" class="btn btn-primary btn-lg btn-block" name="cancel" value="Cancel trade"></form>
                                                            <?php endif;?>
                                                    </td>

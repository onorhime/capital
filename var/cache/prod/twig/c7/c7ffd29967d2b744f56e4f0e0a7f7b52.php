<?php

use Twig\Environment;
use Twig\Error\LoaderError;
use Twig\Error\RuntimeError;
use Twig\Extension\CoreExtension;
use Twig\Extension\SandboxExtension;
use Twig\Markup;
use Twig\Sandbox\SecurityError;
use Twig\Sandbox\SecurityNotAllowedTagError;
use Twig\Sandbox\SecurityNotAllowedFilterError;
use Twig\Sandbox\SecurityNotAllowedFunctionError;
use Twig\Source;
use Twig\Template;

/* dashboard/deposit.html.twig */
class __TwigTemplate_a7950be18c80285f0d1acb52630e87c7 extends Template
{
    private $source;
    private $macros = [];

    public function __construct(Environment $env)
    {
        parent::__construct($env);

        $this->source = $this->getSourceContext();

        $this->blocks = [
            'main' => [$this, 'block_main'],
        ];
    }

    protected function doGetParent(array $context)
    {
        // line 1
        return "base.html.twig";
    }

    protected function doDisplay(array $context, array $blocks = [])
    {
        $macros = $this->macros;
        $this->parent = $this->loadTemplate("base.html.twig", "dashboard/deposit.html.twig", 1);
        yield from $this->parent->unwrap()->yield($context, array_merge($this->blocks, $blocks));
    }

    // line 4
    public function block_main($context, array $blocks = [])
    {
        $macros = $this->macros;
        yield "    <div class=\"container-fluid mb-5\">
                <!-- Title -->
    <h1 class=\"h2\">
        Deposit into your account
    </h1>
    <div wire:id=\"zOhjcaudQZGbFSFczW0V\" wire:initial-data=\"{&quot;fingerprint&quot;:{&quot;id&quot;:&quot;zOhjcaudQZGbFSFczW0V&quot;,&quot;name&quot;:&quot;user.deposit.index&quot;,&quot;locale&quot;:&quot;en&quot;,&quot;path&quot;:&quot;dashboard\\/deposits&quot;,&quot;method&quot;:&quot;GET&quot;,&quot;v&quot;:&quot;acj&quot;},&quot;effects&quot;:{&quot;listeners&quot;:[]},&quot;serverMemo&quot;:{&quot;children&quot;:[],&quot;errors&quot;:[],&quot;htmlHash&quot;:&quot;1073e56c&quot;,&quot;data&quot;:{&quot;methodSelected&quot;:&quot;BUSD&quot;,&quot;amountSelected&quot;:null,&quot;amount&quot;:null},&quot;dataMeta&quot;:[],&quot;checksum&quot;:&quot;eda5abe4f1c63aa21519b8efc26c321c49a4728b85aebed7760fdf36ce5f9a6f&quot;}}\">
    <div>
    </div>
    <div>
    </div>
    <div class=\"row\" x-data>
                    <div class=\"col-6 col-xl-3\">
                <!-- Card -->
                <a  onClick=\"event.preventDefault(); selectMethod('eth')\">
                    <div class=\"card \" id=\"ethcard\">
                        <div class=\"card-body\">
                            <div class=\"d-flex justify-content-between\">
                                <div
                                    class=\"avatar avatar-lg text-bg-gray-300 d-flex align-items-center justify-content-center\">
                                                                            <svg width=\"42px\" height=\"37px\" viewBox=\"0 0 1024 1024\" class=\"icon\"
                                            version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">
                                            <path d=\"M512 512m-480 0a480 480 0 1 0 960 0 480 480 0 1 0-960 0Z\"
                                                fill=\"#E9E8FF\" />
                                            <path
                                                d=\"M467.2 332.8l230.4-83.2 44.8 83.2zM384 332.8l96-83.2 38.4 51.2-64 32z\"
                                                fill=\"#C6C9FF\" />
                                            <path
                                                d=\"M300.8 755.2c-25.6 0-51.2-25.6-51.2-51.2V384c0-25.6 25.6-51.2 51.2-51.2h428.8c25.6 0 51.2 25.6 51.2 51.2v313.6c0 25.6-25.6 51.2-51.2 51.2l-428.8 6.4z\"
                                                fill=\"#8880FE\" />
                                            <path
                                                d=\"M761.6 608H704c-25.6 0-51.2-19.2-51.2-51.2v-19.2c0-25.6 25.6-51.2 51.2-51.2h57.6c25.6 0 51.2 19.2 51.2 51.2v25.6c0 25.6-19.2 44.8-51.2 44.8z\"
                                                fill=\"#C6C9FF\" />
                                        </svg>
                                                                    </div>
                            </div>
                            <!-- Title -->
                            <h3 class=\"card-title mt-3 mb-0 text-dark\">ETH</h3>
                            <span class=\"text-muted m-0 p-0\">Pay via ethereum</span>
                        </div>
                    </div>
                </a>
            </div>
                    <div class=\"col-6 col-xl-3\">
                <!-- Card -->
                <a onClick=\"event.preventDefault(); selectMethod('usdt')\">
                    <div class=\"card\" id=\"usdtcard\">
                        <div class=\"card-body\">
                            <div class=\"d-flex justify-content-between\">
                                <div
                                    class=\"avatar avatar-lg text-bg-gray-300 d-flex align-items-center justify-content-center\">
                                                                            <svg width=\"42px\" height=\"37px\" viewBox=\"0 0 1024 1024\" class=\"icon\"
                                            version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">
                                            <path d=\"M512 512m-480 0a480 480 0 1 0 960 0 480 480 0 1 0-960 0Z\"
                                                fill=\"#E9E8FF\" />
                                            <path
                                                d=\"M467.2 332.8l230.4-83.2 44.8 83.2zM384 332.8l96-83.2 38.4 51.2-64 32z\"
                                                fill=\"#C6C9FF\" />
                                            <path
                                                d=\"M300.8 755.2c-25.6 0-51.2-25.6-51.2-51.2V384c0-25.6 25.6-51.2 51.2-51.2h428.8c25.6 0 51.2 25.6 51.2 51.2v313.6c0 25.6-25.6 51.2-51.2 51.2l-428.8 6.4z\"
                                                fill=\"#8880FE\" />
                                            <path
                                                d=\"M761.6 608H704c-25.6 0-51.2-19.2-51.2-51.2v-19.2c0-25.6 25.6-51.2 51.2-51.2h57.6c25.6 0 51.2 19.2 51.2 51.2v25.6c0 25.6-19.2 44.8-51.2 44.8z\"
                                                fill=\"#C6C9FF\" />
                                        </svg>
                                                                    </div>
                            </div>
                            <!-- Title -->
                            <h3 class=\"card-title mt-3 mb-0 text-dark\">USDT</h3>
                            <span class=\"text-muted m-0 p-0\">Pay via usdt</span>
                        </div>
                    </div>
                </a>
            </div>
            
                    <div class=\"col-6 col-xl-3\">
                <!-- Card -->
                <a onClick=\"event.preventDefault(); selectMethod('btc')\">
                    <div class=\"card\" id=\"btccard\">
                        <div class=\"card-body\">
                            <div class=\"d-flex justify-content-between\">
                                <div
                                    class=\"avatar avatar-lg text-bg-gray-300 d-flex align-items-center justify-content-center\">
                                                                            <img src=\"data:image/png;base64,\" alt=\"\" srcset=\"\" width=\"42px\"
                                            height=\"37px\">
                                                                    </div>
                            </div>
                            <!-- Title -->
                            <h3 class=\"card-title mt-3 mb-0 text-dark\">Bitcoin</h3>
                            <span class=\"text-muted m-0 p-0\">Pay via bitcoin</span>
                        </div>
                    </div>
                </a>
            </div>
            </div>
    <div class=\"row mt-3\">
        <div class=\"col-12\">
            <h1 class=\"m-0 p-0\" id=\"selectedMethod\">
             Select Payment Method
            </h1>
            <span class=\"badge rounded-pill text-bg-primary m-0\">selected</span>
        </div>

        <div class=\"col-lg-8 mt-3\">
            <p class=\"m-0\">Select an amount</p>
            <div class=\"row mt-2\">
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(100)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 100
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(200)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 200
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(400)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 400
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(500)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 500
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(800)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 800
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-4 col-3\">
                    <a onClick=\"setAmount(1000)\">
                        <div class=\"card text-center py-2 fw-bold\">
                            \$ 1000
                        </div>
                    </a>
                </div>
                <div class=\"col-lg-12 mt-2\">
                    <label class=\"visually-hidden\" for=\"inlineFormInputGroupUsername\">Enter amount</label>
                    <form action=\"payment\" method=\"POST\"
                        class=\"row row-cols-lg-auto g-3 align-items-center\">
                        <input type=\"hidden\" name=\"_token\" value=\"MdKz9k7MQVmCr7wqZOjxDsvD9HMX4lohQnFpOuPG\">                        <div class=\"col-12\">
                            <input class=\"form-control w-100\" id=\"amount\" placeholder=\"Enter Amount\"
                                min=\"5\" type=\"number\" step=\"any\" name=\"amount\"
                                 value=\"\" required>
                            <input type=\"hidden\" name=\"method\" value=\"\"
                                 id=\"paymethod\">
                        </div>
                        <div class=\"col-12\">
                            <button type=\"submit\" name='deposit' class=\"btn btn-primary\">
                                Proceed
                                <i class=\"bi bi-arrow-right\"></i>
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
<script>
function selectMethod(value){
    document.getElementById(\"selectedMethod\").textContent = value.toUpperCase();
     document.getElementById(\"paymethod\").value = value;
     document.getElementById(`ethcard`).className = \"card\"
     document.getElementById(`btccard`).className = \"card\"
     document.getElementById(`usdtcard`).className = \"card\"
    document.getElementById(`\${value}card`).classList.add(\"border\")
    document.getElementById(`\${value}card`).classList.add(\"boborder-3\")
    document.getElementById(`\${value}card`).classList.add(\"border-primary\")
    }

function setAmount(value){
    document.getElementById(\"amount\").value = value;
    
    }
</script>
";
        return; yield '';
    }

    /**
     * @codeCoverageIgnore
     */
    public function getTemplateName()
    {
        return "dashboard/deposit.html.twig";
    }

    /**
     * @codeCoverageIgnore
     */
    public function isTraitable()
    {
        return false;
    }

    /**
     * @codeCoverageIgnore
     */
    public function getDebugInfo()
    {
        return array (  47 => 4,  36 => 1,);
    }

    public function getSourceContext()
    {
        return new Source("", "dashboard/deposit.html.twig", "/home/evolvedchain.net/public_html/templates/dashboard/deposit.html.twig");
    }
}
